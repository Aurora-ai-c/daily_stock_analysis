...
class StrategyParser:
    ...
    def _build_strategy(self, raw: dict[str, Any]) -> Strategy:
        ...
        if not isinstance(api_version, int) or api_version < 1:
            raise StrategyParseError(f"api_version must be a positive integer, got {api_version!r}")
        if api_version > _SUPPORTED_API_VERSION:
            raise StrategyParseError(
                f"api_version {api_version} is newer than supported "
                f"({_SUPPORTED_API_VERSION}); upgrade alphaevo"
            )
        ...
        return Strategy(
            ...
            warmup_days=raw.get("warmup_days"),
            meta=meta,
