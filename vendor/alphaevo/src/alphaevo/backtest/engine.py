...
def _has_prev_close(row: Any) -> bool:
    """Return True when a row exposes a usable previous close.

    The A-share limit-up/limit-down rules and the signal probe's
    ``limit_basis: prev_close`` contract both depend on this value.
    """
    try:
        val = row["prev_close"]
...
@dataclass
class LastBarSignal:
    """Point-in-time signal evaluated on the final bar of a symbol's frame.

    Used by the daily signal probe: answers "would this strategy enter today?"
    without running a full backtest. ``triggered=False`` streams are normal —
    the probe reports why the strategy stayed flat.
    """
...
class BacktestEngine:
    ...
    def signal_at_last_bar(
        self,
        strategy: Strategy,
        df: pd.DataFrame,
        symbol: str,
        contexts: dict[str, IndicatorContext] | None = None,
        rules: MarketRuleConfig | None = None,
    ) -> LastBarSignal:
        """Evaluate the entry setup on the final bar of ``df`` (today).

        Deterministic, single-point evaluation reused by the strategy-signal
        probe inside DSA. Does not mutate engine state.

        Args:
            strategy: Strategy to evaluate.
            df: OHLCV DataFrame with the same schema as :meth:`run`
                (must include ``prev_close`` for A-share limit-up checks).
            symbol: Symbol under evaluation.
            contexts: Optional per-symbol IndicatorContext for L2/L3 indicators.
            rules: Pre-resolved MarketRuleConfig; defaults to strategy config
                for the strategy's market.

        Returns:
            LastBarSignal describing whether the strategy fires today.
        """
        market_key = strategy.meta.market.value
        resolved_rules = rules or strategy.market_rules.get(
            market_key,
            MarketRuleChecker.default_rules(market_key),
        )
        ctx = contexts.get(symbol) if contexts else None

        as_of_date = self._get_date(df, len(df) - 1)
        if df is None or df.empty or len(df) < self.min_data_days:
            return LastBarSignal(
                symbol=symbol,
                as_of_date=as_of_date,
                strategy_id=strategy.meta.id,
                triggered=False,
                reason=f"insufficient data: {0 if df is None else len(df)} bars",
                insufficient_data=True,
            )
        if strategy.warmup_days and len(df) < strategy.warmup_days:
            return LastBarSignal(
                symbol=symbol,
                as_of_date=as_of_date,
                strategy_id=strategy.meta.id,
                triggered=False,
                reason=f"insufficient data for warmup_days={strategy.warmup_days}: {len(df)} bars",
                insufficient_data=True,
            )

        last_idx = len(df) - 1

        # 1. Market rule gate (limit-up / suspension) with diagnostics
        if not self.rule_checker.can_buy(df, last_idx, resolved_rules):
            row = df.iloc[last_idx]
            prev = row["prev_close"] if "prev_close" in df.columns else None
            limit_up = False
            if resolved_rules.limit_up_down and _has_prev_close(row):
                try:
                    limit_up = self.rule_checker._is_limit_up(row)
                except Exception:
                    limit_up = False
            volume = row["volume"] if "volume" in df.columns else 0
            suspended = resolved_rules.suspension and volume == 0
            if limit_up:
                return LastBarSignal(
                    symbol=symbol,
                    as_of_date=as_of_date,
            ...
            return LastBarSignal(
                ...
                strategy_id=strategy.meta.id,
                triggered=False,
                reason="market_rule_blocks_buy",
                limit_up=limit_up,
                limit_basis="prev_close",
            )

        # 2. Regime gate
        if not self._regime_allows_entry(strategy, df, last_idx, ctx):
            return LastBarSignal(
                symbol=symbol,
                as_of_date=as_of_date,
                strategy_id=strategy.meta.id,
                triggered=False,
                reason="regime_disinclined",
                limit_basis="prev_close",
            )

        # 3. Entry evaluation on the last bar
        if not self.evaluator.evaluate_entry(strategy.entry, df, last_idx, ctx):
            return LastBarSignal(
                symbol=symbol,
                as_of_date=as_of_date,
                strategy_id=strategy.meta.id,
        ...
        # 4. Entry price basis: next_open cannot be observed on the last bar;
        #    fall back to last close and flag the basis explicitly.
        timing = "next_open"
        if strategy.entry.execution and strategy.entry.execution.timing:
            timing = strategy.entry.execution.timing
        entry_price: float | None = None
        entry_basis: str | None = None
        if timing == "close":
            entry_price = float(df["close"].iloc[last_idx])
            entry_basis = "last_close"
        else:
            entry_price = float(df["close"].iloc[last_idx])
            entry_basis = "last_close_approx_next_open"

        limit_up = False
        limit_down = False
        if resolved_rules.limit_up_down and _has_prev_close(df.iloc[last_idx]):
            try:
                limit_up = self.rule_checker._is_limit_up(df.iloc[last_idx])
                limit_down = self.rule_checker._is_limit_down(df.iloc[last_idx])
            except Exception:
                pass
        ...
        return LastBarSignal(
            ...
            limit_basis="prev_close",
            indicator_snapshot=self._capture_indicators(strategy, df, last_idx, ctx),
        )
    ...
    def _run_symbol(
        ...
        # (strategy-declared warmup_days takes precedence when present)
        warmup = max(
            self.min_data_days,
            strategy.warmup_days or 0,
            _indicator_warmup(self._collect_all_indicators(strategy)),
        )
        start_idx = warmup
