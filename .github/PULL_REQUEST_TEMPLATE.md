...
>
> - Playwright 截图产物：`apps/client/web/e2e/smoke.spec.ts`（`cd apps/client/web && npx playwright test e2e/smoke.spec.ts --grep "settings page renders title and save actions after login"`）
> - 审查证据链接：可直接使用 Actions 产物、GitHub 评论附件或外部可访问链接。

> 替代证据模板（设置页变更建议）：
> - 命令：`cd apps/client/web && npx playwright test e2e/smoke.spec.ts --grep "settings page"`
> - 产物路径：`apps/client/web/test-results/**/smoke-settings-page-*.png`
...
- 不适用原因 / Reason if not applicable（若未附截图，此项务必填写，且包含可复现证据与命令）：
  - Playwright 命令（无截图时）：`cd apps/client/web && npx playwright test e2e/smoke.spec.ts --grep "settings page"`
  - 产物路径（无截图时）：`apps/client/web/test-results/**/smoke-settings-page-*.png`
