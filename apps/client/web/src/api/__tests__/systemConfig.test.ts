...
import { buildTestPayload, getProvider } from '../../components/setup/providerConfig';
...
describe('systemConfigApi', () => {
  ...
  it('wizard buildTestPayload is converted to snake_case on the wire (no silent key drop)', async () => {
    const payload = buildTestPayload(getProvider('deepseek'), 'sk-d', 'deepseek-chat');
    await systemConfigApi.testLLMChannel(payload);

    const [, body] = post.mock.calls[0];
    expect(body.api_key).toBe('sk-d');
    expect(body.base_url).toBe('https://api.deepseek.com/v1');
    expect(body.api_surface).toBe('chat_completions');
    expect(body.use_saved_secret).toBe(false);
    // camelCase keys must not leak (would be ignored by pydantic -> unauthenticated test)
    expect(body.apiKey).toBeUndefined();
    expect(body.baseUrl).toBeUndefined();
    expect(body.apiSurface).toBeUndefined();
  });
