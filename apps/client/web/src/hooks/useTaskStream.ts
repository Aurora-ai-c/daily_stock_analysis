...
function reportActiveAnalysis() {
  const dsa = (window as unknown as { dsa?: { setActiveAnalysis?: (value: boolean) => void } }).dsa;
  dsa?.setActiveAnalysis?.(activeTaskIds.size > 0);
}

// Recompute the active-analysis flag from the full task list rather than
// trusting incremental SSE events, which can drift after a reconnect/replay.
async function resyncActiveAnalysis() {
  try {
    const res = await analysisApi.getTasks({ limit: 200 });
    const tasks = (res && Array.isArray((res as { tasks?: unknown[] }).tasks))
      ? (res as { tasks: Array<{ taskId?: string; status?: string }> }).tasks
      : [];
    const next = new Set<string>();
    for (const task of tasks) {
      if (task && (task.status === 'pending' || task.status === 'processing')) {
        if (task.taskId) next.add(task.taskId);
      }
    }
    activeTaskIds = next;
    reportActiveAnalysis();
  } catch (e) {
    console.warn('[useTaskStream] failed to resync active analysis state', e);
  }
}
...
function connectSharedStream() {
  ...
  eventSource.addEventListener('task_started', (e) => {
    const payload = parseEventData((e as MessageEvent<string>).data);
    if (payload) {
      if (payload.task.taskId) activeTaskIds.add(payload.task.taskId);
      reportActiveAnalysis();
  ...
  eventSource.addEventListener('task_completed', (e) => {
    const payload = parseEventData((e as MessageEvent<string>).data);
    if (payload) {
      activeTaskIds.delete(payload.task.taskId);
      reportActiveAnalysis();
