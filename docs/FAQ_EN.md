...
3. The `version` field in `apps/client/web/package.json` is currently a placeholder `0.0.0`. The WebUI version/build card is useful for checking whether frontend assets were rebuilt, but it is not the Docker release version.
4. The desktop app has its own version in `apps/client/electron/package.json`, and that only applies to the Electron desktop build, not the Docker image.
