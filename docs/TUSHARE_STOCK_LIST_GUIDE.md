...
新版客户端默认会从项目 GitHub `main` 分支读取最新的 `apps/client/web/public/stocks.index.json`，并缓存到本地 `data/cache/stocks.index.json`。前端仍访问本地 `/stocks.index.json`，不需要直接跨域请求 GitHub。
