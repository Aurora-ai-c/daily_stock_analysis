...
- 当前仓库对此链路的依赖约束是 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`（见 `requirements.txt`）；回归覆盖包括 `tests/test_system_config_service.py`、`tests/test_system_config_api.py` 和 `apps/client/web/src/components/settings/__tests__/LLMChannelEditor.test.tsx`。
...
- 前端回归命令：`cd apps/client/web && npm run lint && npm run build && npm run test -- src/components/settings/__tests__/LLMChannelEditor.test.tsx`。
...
- 回归验证入口：
  ...
  - 设置页交互与保存后提示：`apps/client/web/src/components/settings/__tests__/LLMChannelEditor.test.tsx`
